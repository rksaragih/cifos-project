...
const MenuAdmin = () => {
    ...
    fetch('/api/menus')
      ...
      .then((data) => {
        const mapped = data.map((m: any) => ({
          ...
          kategori_id: m.kategori_id ?? m.kategori_id ?? (m.category?.id ?? undefined),
  ...
  useEffect(() => {
    const input = document.getElementById('menu-file-input') as HTMLInputElement | null;
    if (!input) return;
    ...
    const handler = async (e: Event) => {
      ...
      try {
        ...
        toast({ title: 'Uploaded', description: 'Foto berhasil diunggah' });
      } catch (err: any) {
        toast({ title: 'Upload error', description: err?.message || 'Gagal mengunggah file', variant: 'destructive' });
      } finally {
        setUploading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.kategori_id || !form.nama_menu || !form.harga_menu) {
      toast({ title: 'Error', description: 'Lengkapi kategori, nama, dan harga', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      toast({ title: 'Berhasil', description: editingId ? 'Menu berhasil diupdate' : 'Menu berhasil ditambahkan' });
      setShowForm(false);
      setForm({ kategori_id: '', nama_menu: '', harga_menu: '', foto_menu: '', tersedia: true, rekomendasi: false, best_seller: false });
      setEditingId(null);
    } catch (err: any) {
      toast({ title: 'Error', description: err?.message || 'Gagal menyimpan menu', variant: 'destructive' });
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/menus/${id}`, { method: 'DELETE', credentials: 'include', headers: { 'Accept': 'application/json' } });
      if (!res.ok) throw new Error('Gagal menghapus menu');
      setItems(prev => prev.filter(p => p.id !== id));
      toast({ title: 'Deleted', description: 'Menu berhasil dihapus' });
    } catch (err: any) {
      toast({ title: 'Error', description: err?.message || 'Gagal menghapus menu', variant: 'destructive' });
    }
  };
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Kelola daftar menu — foto, kategori, harga, dan status ketersediaan</p>
        </div>
          ...
          <tbody>
            {items
              .filter((it) => {
                ...
                return String(it.nama_menu).toLowerCase().includes(q) || String(it.kategori).toLowerCase().includes(q);
              })
              .map(it => (
              <tr key={it.id} className="border-t even:bg-gray-50">
                <td className="p-3 w-20">
                  {it.foto_menu ? <img src={it.foto_menu} alt={it.nama_menu} className="w-16 h-12 object-cover rounded-md" /> : <div className="w-16 h-12 bg-gray-100 rounded-md" />}
                </td>
                <td className="p-3">{it.kategori}</td>
                <td className="p-3">{it.nama_menu}</td>
                <td className="p-3">Rp {Number(it.harga_menu || 0).toLocaleString('id-ID')}</td>
                <td className="p-3">{it.tersedia ? <span className="inline-block px-2 py-1 text-xs bg-green-100 text-green-800 rounded-full">Tersedia</span> : <span className="inline-block px-2 py-1 text-xs bg-red-100 text-red-800 rounded-full">Tidak tersedia</span>}</td>
                <td className="p-3">{it.rekomendasi ? <span className="inline-block px-2 py-1 text-xs bg-yellow-100 text-yellow-800 rounded-full">Yes</span> : '-'}</td>
                <td className="p-3">{it.best_seller ? <span className="inline-block px-2 py-1 text-xs bg-purple-100 text-purple-800 rounded-full">Yes</span> : '-'}</td>
                <td className="p-3 text-right">
                  <Button size="sm" variant="outline" onClick={() => handleEdit(it)}>Edit</Button>
                  <Button size="sm" className="ml-2" onClick={() => handleDelete(it.id)}>Delete</Button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
        <div className="p-3 bg-gray-50 text-sm text-muted-foreground">Showing {items.length} items</div>
      </div>
    </div>
  );
};
